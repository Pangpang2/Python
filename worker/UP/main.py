...
import worker.UP.jenkins_caller
import time
...
if __name__ == '__main__':
    ...
    check_list = ['Diagnosis', Entity.TRANSACTION]
    ...
    # Integration.upload_data(auto_account, scope, Action.INSERT, env, clear=True, check_list=check_list)
    ...
    # UploadProcessor.clear_table_for_business(dental_account.business_id, 'Appointment')
    # UploadProcessor.clear_table_for_business(dental_account.business_id, 'Customer')

    # UploadProcessor.clear_table_for_business(dental_account.business_id, 'CustomerOptions')
    #####  rember to update the license key in the file(not used)             ######
    ######  account only need to update business id and license key ###########
    # UploadProcessor.clear_table_for_business(dental_account.business_id, 'UploadHistory')
    # UploadProcessor.clear_table_for_business(dental_account.business_id, 'Customer')
    # UploadProcessor.clear_table_for_business(dental_account.business_id, 'Appointment')
    # Integration.upload_data_with_file(dental_account, 'template',
    #                                   'test.xml',
    #                                   env, clear=False)
    # print(UploadProcessor.get_customer_from_db(dental_account.business_id))
    # print(UploadProcessor.get_appointment_from_db(dental_account.business_id))
    # print(UploadProcessor.get_customer_from_db(dental_account.business_id))
